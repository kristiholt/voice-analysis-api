...
class PostProcessor:
    ...
    def _apply_legacy_scale_mapping(self, scores: Dict[str, float]) -> Dict[str, float]:
        """
        Apply legacy scale mapping to ensure all scores are in 0-100 range.
        
        Handles different score types:
        - Base emotions (emo1-emo26): Scale from [0,1] to [0,100]
        - Meta-emotions (emo29+): Already in [0,100], just clamp
        - Traits (char1-char94): Scale from [0,1] to [0,100]
        """
        legacy_scores = {}
        
        for key, value in scores.items():
            if key.startswith('emo') and len(key) > 3 and key[3:].isdigit():
                emo_num = int(key[3:])
                if emo_num >= 29:
                    # Meta-emotions: already in 0-100 range, just clamp
                    legacy_value = max(0.0, min(100.0, value))
                else:
                    # Base emotions: scale from [0,1] to [0,100]
                    clamped_value = max(0.0, min(1.0, value))
                    legacy_value = clamped_value * 100.0
            else:
                # Traits and other scores: scale from [0,1] to [0,100] 
                clamped_value = max(0.0, min(1.0, value))
                legacy_value = clamped_value * 100.0
